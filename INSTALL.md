...
[tunnel-client Windows amd64版ZIPをダウンロード](https://github.com/openai/tunnel-client/releases/latest/download/tunnel-client-v0.0.14-windows-amd64.zip)<br>
このリンクを開くとZIPのダウンロードが始まります。<br>ブラウザーのダウンロード先へ`tunnel-client-v0.0.14-windows-amd64.zip`が保存されたことを確認します。<br>
...
`gitmcp`を有効にすると、`roots`、`get_working_directory`、`set_working_directory`、`status`、`ls_files`、`list_worktree_files`、`check_ignore`、`check_attributes`、`get_effective_config`、`branches`、`tags`、`remotes`、`log`、`diff`、`show`、`switch_branch`、`create_branch`、`checkout`、worktree操作を公開します。`add_all`、`stage_paths`、`unstage_paths`、`commit`、`push`、`pull`、`clone_repository`は公開しません。旧`--disable-push`、`--disable-pull`、`--disable-clone`は古い設定を起動不能にしないためno-opとして受理します。<br>
...
`push`、`pull`、`clone`も同じ`mcp\git-capability\server.mjs`を別MCPとして登録します。push / pullは`--remote=origin`と`--repository=OWNER/REPO`を起動時に固定し、実際のremote URLをGitHubのrepository identityへ正規化して照合します。旧`--expected-remote-url=https://github.com/OWNER/REPO.git`も下位互換性のため利用できます。cloneは起動時`--url`を持たず、`clone_repository`呼び出しごとに`http://`、`https://`、`ssh://user@host/path`、`user@host:path`形式のURLを指定します。通常のGit credential helper / askpass / SSH認証を使えるようnetwork capabilityでは認証関連環境を維持します。cloneはsubmodule再帰を提供しません。詳細な4 modeの設定例は`config/gateway.example.toml`と`config/gateway.codex.example.toml`を参照してください。<br>
...
分離で、Gateway側で明示的に許可されていない場合は、gateway__get_configで代替パスがないか調べて、そこを使え。即故障扱いにするな。ユーザ指示ルートでしか作業してはいけないというルールを作るな
===終了ローカルMCPプラグイン指示===
```
