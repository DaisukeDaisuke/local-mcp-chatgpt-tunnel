...
const workspaceRoot = (prefix = 'dq9-suite-root-') => mkdtemp(join(tmpdir(), prefix));

test('DQ9 suites are read only from configured workspace roots', async () => {
  ...
  await assert.rejects(loadAllowedSuiteFile(outside, { roots: [root] }), /outside all configured workspace roots/);
});
...
test('DQ9 suite policy rejects credential paths, symlink escapes, secrets, and UTF-16', async () => {
  ...
  await assert.rejects(loadAllowedSuiteFile(link, { roots: [root] }), /escaped the configured workspace root/);
