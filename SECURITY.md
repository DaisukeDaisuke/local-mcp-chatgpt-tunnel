...
safe-filesは`cwd`を相対パスの初期基準とし、設定された`allowed_directories`だけをWorkspaceとして扱います。`set_working_directory`はその許可範囲内だけへ変更でき、許可範囲外とシンボリックリンクによる脱出を拒否します。`copy`と`move`は送信元と送信先の両方へ同じ許可・拒否ポリシーを適用し、通常ファイルだけを扱い、既存送信先を上書きしません。移動は排他的な複製後に送信元を削除し、削除失敗時は作成した送信先を除去します。パスはシェルへ渡さず、記号を命令として解釈しません。固定の一般ブラックリストは持たず、利用者が`disallowed_path_globs`へ明示した拒否パターンだけを適用します。高確度の秘密文字列は内容検査で拒否し、`.git`内部へのpatchは操作固有の制約として拒否します。
...
gh-workflowはActionsの読み取り操作と、明示されたrunのキャンセルだけを公開し、起動時に1件以上の`--repository=OWNER/REPO`を必須とします。複数指定した場合も、その許可リスト内だけを各ツールの`repository`として選択できます。`gh`は明示した`cwd`から`spawn('gh', args, { shell:false })`で直接起動し、標準入力を閉じ、サブコマンドとオプションを固定し、run ID、branch、workflow識別子を個別に検証します。`cancel_run`は検証済みrun IDを`gh run cancel RUN_ID --repo OWNER/REPO`の固定引数へ渡すだけです。workflow実行、再実行、delete、artifact download、`gh api`、任意gh引数は公開しません。
