...
`ssh`はremote commandを文字列1本ではなくtoken配列で受け取り、空白、quote、`!`、`@`、backtick、`$`、`;`、`&`、pipeなどshell expansion / metacharacterを拒否します。同期SSHの実行時間は最大10分です。`async = true`を最初から指定した場合は即座に`asyncId`を返し、`timeoutMs`でhard runtimeを最大1時間まで指定できます。`timeoutMs`を省略した場合は同期/asyncとも既定10分です。`get_async_status`、`get_async_logs`、`write_async_stdin`、`wait_async`、`cancel_async`で状態確認、stdout/stderr取得、stdin送信/EOF、再待機、中断ができます。`write_async_stdin`は1回64 KiB、1job累積1 MiBまでで、`end = true`を送った後は再送できません。`wait_async`の1回の待機は最大10分です。wait timeoutやstdin送信はジョブ実行期限を延長しません。実行中jobは16件まで、保持jobは64件までです。<br>
...
`list_temporary_public_deployments`は、GitHub側ですでに認識されているCodespaceの一時公開候補と`browseUrl` / port / visibilityを取得します。これはlocalhost、ローカルPCの待受socket、ブラウザタブ、ローカル開発サーバーを走査するツールではなく、portの自動検出も行いません。GitHub側の候補が0件の場合は空配列を正常結果として返さず、「これはローカルport自動検出の失敗ではない」「localhost探索・port scan・URL推測・`gh codespace ports forward`への迂回をしない」ことを明示する補正エラーを返します。`open_temporary_public_deployment`は呼び出し側が明示した1つのCodespace portだけを`public`へ変更し、完全な`https://...app.github.dev` URLを返します。指定portがGitHub側に存在しない場合も、localhostを探索して代替portを推測せずに失敗します。`close_temporary_public_deployment`も明示された同じportだけを`private`へ戻して一時公開を閉じます。いずれもlocalhost port tunnelを作らず、GitHub側のforward entryそのものも新規作成・削除しません。返された`browseUrl`はそのままChrome DevTools等から一時デプロイの疎通確認に使えます。<br>
### gitmcp
