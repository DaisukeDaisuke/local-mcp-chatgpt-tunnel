...
| `codespace` | `list_codespaces`、`roots`、`git_root`、`search_text`、`ssh`、`copy_to_codespace`、`open_port`、`close_port` | 既存GitHub Codespaceだけを対象に、remote検索・SSH・転送・GitHub-hosted preview port公開を行う。Codespace作成ツールは持たない |
...
### codespace
`codespace`は、すでに存在するGitHub Codespaceだけを操作する同梱MCPです。`list_codespaces`が返した`name`を各ツールの`codespaceId`として使います。Codespaceのcreate、rebuild、machine変更、明示的なstart/stop、deleteを行うツールは実装しません。AIが気まぐれにCodespaceを乱立できる経路はありません。同じ`codespaceId`は1つのisolated sessionだけが所有し、別の新しいisolated sessionが触れた場合は後勝ちで所有権を移します。旧sessionは自動で奪い返せず、競合エラーから`isolated__list`の`purpose`と`codespace` prefixの`lastOperationAt`を確認してユーザーへ判断を求めるよう指示されます。同一`codespaceId`の呼び出しだけGatewayで直列化し、files等の別bundled MCPは不要に直列化しません。<br>
MCP自体は必ず`sandbox = "onlineworkspace"`で起動します。`--gh-executable=<absolute-gh.exe-path>`は必須です。Windows Credential Managerへ保存された通常の`gh auth login`資格情報がsandbox userから見えない構成に備え、任意で`--token-file=<absolute-file>`を指定できます。このファイルは固定read-only trust inputとしてCodex permission profileへ渡され、内容はMCP内部だけで`GH_TOKEN`へ設定します。親環境の`GH_TOKEN` / `GITHUB_TOKEN`は継承しません。SSH鍵も任意の`--ssh-key-file=<absolute-private-key>`を固定read-only trust inputとして指定できます。これらの固定ファイルと`gh.exe`はwrite可能な`allowed_directories`内に置けません。<br>
`ssh`はremote commandを文字列1本ではなくtoken配列で受け取り、空白、quote、`!`、`@`、backtick、`$`、`;`、`&`、pipeなどshell expansion / metacharacterを拒否します。同期SSHも実行時間は最大10分です。長時間かかる可能性がある場合は`async = true`を指定すると即座に`asyncId`を返します。`get_async_status`、`get_async_logs`、`write_async_stdin`、`wait_async`、`cancel_async`で状態確認、stdout/stderr取得、stdin送信/EOF、再待機、中断ができます。`write_async_stdin`は1回64 KiB、1job累積1 MiBまでで、`end = true`を送った後は再送できません。非同期ジョブ自体のhard runtimeも最大10分で、`wait_async`の1回の待機も最大10分です。wait timeoutやstdin送信はジョブ実行期限を延長しません。実行中jobは16件まで、保持jobは64件までです。<br>
`copy_to_codespace`は1対1の任意destination mappingを公開せず、1つの`sourceDirectory`配下から`paths`列挙または`globs`のどちらかで複数対象を選び、1つの`remoteDestination`ディレクトリへまとめて送ります。そのisolated sessionで対象CodespaceへのSSH成功をまだ確認していない場合だけ固定`echo started` probeを1回行い、以後はreadinessを再利用します。明示的な同期`ssh`成功もreadinessとして再利用されます。readiness再利用中に`cp`が失敗した場合だけcacheを捨て、probe後に`cp`を1回だけ再試行します。probeのstdout/stderrは`startupStdout` / `startupStderr`として返します。GitHub CLIでは`-e`なしの`gh codespace cp`が、SSHでは存在確認できるremote pathでも`No such file or directory`になる既知問題があるため、このMCPは`gh codespace cp -e`を常用します。`-e`自体を避けるのではなく、remote destinationの構文だけを固定し、絶対pathまたは`~/...`の安全な文字に限定してquote、`!`、`@`、`$`、backtick、shell演算子、glob記号を拒否します。転送量は`CODESPACE_MCP_MAX_TRANSFER_BYTES`で制限し、設定値そのものも500,000,000 bytesを超えられません。選択した総量が上限以上なら`gh cp`前に拒否します。<br>
...
`list_ports`はGitHub側でforward済みのportと`browseUrl` / visibilityを取得します。`open_port`は指定portを`public`へ変更し、完全な`https://...app.github.dev` URLを返します。これはlocalhostへのport tunnelではありません。`close_port`は同じportを`private` visibilityへ戻し、public deploymentを閉じます。GitHub側のforward entryそのものを削除する操作ではありません。返された`browseUrl`はそのままChrome DevTools等から開いてpreviewの疎通確認に使えます。<br>
### gitmcp
